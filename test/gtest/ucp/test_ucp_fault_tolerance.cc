...
/** Active Message ID and payload size for AM fault tolerance test */
static const unsigned AM_ID       = 0;
static const size_t   AM_MSG_SIZE  = 1 * UCS_KBYTE;
...
protected:
    ...
    static ucs_status_t am_recv_cb(void *arg, const void *header,
        ...
        if (param->recv_attr & UCP_AM_RECV_ATTR_FLAG_DATA) {
            memcpy(self->m_am_rbuf, data, length);
        ...
        if (param->recv_attr & UCP_AM_RECV_ATTR_FLAG_RNDV) {
            ...
            self->m_am_rreq = ucp_am_recv_data_nbx(self->receiver().worker(),
                                                   data, self->m_am_rbuf,
                                                   AM_MSG_SIZE, &rparam);
    ...
    void test_am_with_injected_failure(failure_side_t failure_side, bool flush_after = false) {
        ...
        ucs_carray_for_each(am_bw_lane_idx,
        ...
        if (am_bw_lanes.size() < 2) {
            UCS_TEST_SKIP_R("At least 2 AM BW lanes are required, but only " +
                            std::to_string(am_bw_lanes.size()) + " available");
        }
        ...
        { // allocate randomizer on heap to avoid exceeding stack frame size limits
            ...
            std::shuffle(am_bw_lanes.begin(), am_bw_lanes.end(), *rng);
        }

        for (ucp_lane_index_t lane : am_bw_lanes) {
            UCS_TEST_MESSAGE << "AM BW lane: " << size_t(lane) << "/" << am_bw_lanes.size();
        }
        ...
        EXPECT_EQ(UCS_OK, status) << "AM send returned status: "
...
private:
    ...
    ucs_status_t do_am_send_and_wait(ucp_ep_h ep, size_t size, bool flush_after) {
        ...
        if (flush_after) {
            status = request_wait(ucp_ep_flush_nbx(ep, &param));
            if (status != UCS_OK) {
                return status;
            }
        }
        ...
        while (!m_am_received && (m_am_rreq == nullptr)) {
            short_progress_loop();
        }

        if (m_am_rreq != nullptr) {
            status = request_wait(m_am_rreq);
            m_am_rreq = nullptr;
            if (status != UCS_OK) {
                return status;
            }
        ...
        mem_buffer::pattern_check(m_am_rbuf, size, m_seed);
        return UCS_OK;
    }
...
protected:
    ...
    /* AM test state (used when has_am_feature()) */
    char          m_am_rbuf[AM_MSG_SIZE];
    volatile bool m_am_received = false;
    void         *m_am_rreq    = nullptr;
