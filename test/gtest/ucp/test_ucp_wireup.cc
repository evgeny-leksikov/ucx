...
protected:
    ...
    bool is_close_sync_stream_test() const {
        return GetParam().variant == TEST_STREAM_CLOSE_SYNC;
    }
...
void* test_ucp_wireup::ep_disconnect(ucp_ep_h ep) {
    ...
    unsigned mode = is_close_sync_stream_test() ?
                    UCP_EP_CLOSE_MODE_STREAM_SYNC : UCP_EP_CLOSE_MODE_FLUSH;
    void *req = ucp_ep_close_nb(ep, mode);
    if (!UCS_PTR_IS_PTR(req)) {
        ASSERT_UCS_OK(UCS_PTR_STATUS(req));
    }

    if (mode == UCP_EP_CLOSE_MODE_STREAM_SYNC) {
        return req;
    }
