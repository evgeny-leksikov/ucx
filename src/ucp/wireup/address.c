...
static size_t ucp_address_worker_name_size(ucp_worker_h worker, int no_debug)
{
#if ENABLE_DEBUG_DATA
    return no_debug ? 0 : strlen(ucp_worker_get_name(worker)) + 1;
#else
    return 0;
#endif
}
                                        ...
                                        ucp_rsc_index_t num_devices)
{
    ...
    for (dev = devices; dev < devices + num_devices; ++dev) {
        ...
        *(uint8_t*)ptr = md_index |
        ...
        dev_addr_len = (flags & UCP_ADDRESS_PACK_FLAG_DEVICE_ADDR) ?
                       dev->dev_addr_len : 0;
        ucs_assert(dev_addr_len < UCP_ADDRESS_FLAG_LAST);
        *(uint8_t*)ptr = dev_addr_len | ((dev == (devices + num_devices - 1)) ?
        ...
        ucp_address_memchek(ptr, dev_addr_len,
